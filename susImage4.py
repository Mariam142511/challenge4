    ...
    def listener_callbackFunction(self, imageMessage):
        ...
        for idx, mask in enumerate(masks):
            ...
            circles = cv2.HoughCircles(
                cimg, cv2.HOUGH_GRADIENT, 1, 100,
                param1=50, param2=22, minRadius=20, maxRadius=100
            )

            if circles is not None:
                color_detected = ['Yellow', 'Red', 'Green'][idx]
                self.get_logger().info(f"{color_detected} detected")
                self.msg.data = color_detected
...
# -------- FUNCIÓN MAIN NECESARIA PARA QUE EL NODO FUNCIONE --------
def main(args=None):
    rclpy.init(args=args)
    node = SuscriberNodeClass()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
